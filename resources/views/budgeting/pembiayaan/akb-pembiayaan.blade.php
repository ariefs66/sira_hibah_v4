    ...
    <div class="col">
      <div class="app-content-body ">
        ...
        <div class="bg-light lter">    
          ...
          <li><i class="fa fa-angle-right"></i>Pembiayaan</li>                               
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="panel bg-white">
              <div class="wrapper-lg">
                <h5 class="inline font-semibold text-orange m-n ">AKB PEMBIAYAAN | SKPD : {{$skpd->SKPD_NAMA}} </h5>
                @if($thp == 0)
                ...
                @else
                <a class="pull-right btn m-t-n-sm btn-success" href="{{url('/')}}/main/{{$tahun}}/{{$status}}/lampiran/akb/pembiayaan/{{$skpd->SKPD_ID}}" target="_blank">Print AKB</a>
                @endif
            ...
            <!-- Tab1 -->
              <div role="tabpanel" class="active tab-pane" id="tab-1">  
                <div class="table-responsive dataTables_wrapper">
                 ...
                 sAjaxSource: '{{ url('/') }}/main/{{ $tahun }}/{{ $status }}/pembiayaan/data/akb/{{ $skpd->SKPD_ID }}',
                 aoColumns: [
...
<script type="text/javascript">
  ...
  function simpanAKB(){
    var id_pembiayaan    = $('#id_pembiayaan').val();
   ...
   selisih = total-total_akb;
    ...
    if(jan == "" || feb == "" || mar == "" || apr == "" || mei == "" || jun == "" || jul == "" || agu == "" || sep == "" || nov == "" || des == "" ){
      ...
      if(total != total_akb){
        ...
        $.alert("Total Rek Pembiayaan : <b>"+total+"</b>");
        $.alert("Total Input AKB : <b>"+total_akb+"</b>");
        $.alert("selisih : <b>"+selisih+"</b>");
      }
      else uri = "{{ url('/') }}/main/{{ $tahun }}/{{ $status }}/pembiayaan/akb/ubah";
  ...
  function ubah(PEM_ID, REKENING_ID){
    $('#judul-form').text('Ubah Anggaran Kas Bulanan');
    $.ajax({
      type  : "get",
      url   : "{{ url('/') }}/main/{{ $tahun }}/{{ $status }}/pembiayaan/akb/detail/"+PEM_ID+"/"+REKENING_ID,
      success : function (data) {
        console.log(data);
        $('#id_pembiayaan').val(data['PEMBIAYAAN_ID']);
    ...
    $.confirm({
      ...
      buttons: {
        Ya: {
          ...
          action: function(){
            $.ajax({
              url: "{{ url('/') }}/main/{{ $tahun }}/{{ $status }}/pembiayaan/akb/hapus",
              type: "POST",
              data: {'_token'         : token,
