...
class blController extends Controller
{
    ...
    public function showDetail($tahun,$status,$id){
        ...
        $tahapan    = Tahapan::where('TAHAPAN_TAHUN',$tahun)
            ...
            ->orderBy('TAHAPAN_ID','desc')->first();
        $anggaran = TahunAnggaran::where('TAHUN',$tahun)->where('STATUS',$status)->first();
        $mode =  $tahapan->TAHAPAN_NAMA;
        if($now > $tahapan->TAHAPAN_AWAL && $now < $tahapan->TAHAPAN_AKHIR){
            $thp    = 1;
        }else{
            $thp    = 0;
        ...
        //                 })->sum('RINCIAN_TOTAL');
        $totalRKPD  = RekapRincian::where('BL_ID',$id)->where('TAHAPAN_ID',$anggaran->TAHAP_1)->sum('RINCIAN_TOTAL');
        ...
        //                 })->sum('RINCIAN_TOTAL');
        $totalPPAS  = RekapRincian::where('BL_ID',$id)->where('TAHAPAN_ID',$anggaran->TAHAP_2)->sum('RINCIAN_TOTAL');
        ...
        //                 })->sum('RINCIAN_TOTAL');
        $totalRAPBD = RekapRincian::where('BL_ID',$id)->where('TAHAPAN_ID',$anggaran->TAHAP_3)->sum('RINCIAN_TOTAL');
...
<a class="dropdown-toggle" type="button" id="dropdownMenu2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="text text-success"><i class="fa fa-chevron-down"></i></span></a><ul class="dropdown-menu" aria-labelledby="dropdownMenu2"><li><a onclick="return info(\''.$data->RINCIAN_ID.'\')"><i class="fa fa-info-circle"></i>Info-</a></li></ul></div>';
    ...
    public function restoreRincianCBmurni($tahun,$status){
        $id             = Input::get('RINCIAN_ID');
        foreach ($id as $id) {
            $rincian       = RincianArsip::where('ARSIP_ID',$id)->first();
            $rincianlama       = Rincian::where('RINCIAN_ID',$rincian->RINCIAN_ID)->first();
            Rincian::where('RINCIAN_ID',$rincian->RINCIAN_ID)->delete();
            ...
            $tahapan          = $rincian->TAHAPAN_ID;
            $arcRincian->save();
            $komponen      = Komponen::where('KOMPONEN_ID',$rincian->KOMPONEN_ID)->first();
            RincianArsip::where('ARSIP_ID',$id)->delete();
            if($rincianlama){
                $arcRincian     = new RincianArsip;
                $arcRincian->RINCIAN_ID             = $rincianlama->RINCIAN_ID;
                $arcRincian->SUBRINCIAN_ID          = $rincianlama->SUBRINCIAN_ID;
                $arcRincian->REKENING_ID            = $rincianlama->REKENING_ID;
                $arcRincian->KOMPONEN_ID            = $rincianlama->KOMPONEN_ID;
                $arcRincian->RINCIAN_PAJAK          = $rincianlama->RINCIAN_PAJAK;
                $arcRincian->RINCIAN_VOLUME         = $rincianlama->RINCIAN_VOLUME;
                $arcRincian->RINCIAN_KOEFISIEN      = $rincianlama->RINCIAN_KOEFISIEN;
                $arcRincian->RINCIAN_TOTAL          = $rincianlama->RINCIAN_TOTAL;
                $arcRincian->RINCIAN_KETERANGAN     = $rincianlama->RINCIAN_KETERANGAN;
                $arcRincian->PEKERJAAN_ID           = $rincianlama->PEKERJAAN_ID;
                $arcRincian->BL_ID                  = $rincianlama->BL_ID;
                $arcRincian->RINCIAN_KOMPONEN       = $rincianlama->RINCIAN_KOMPONEN;
                $arcRincian->RINCIAN_HARGA          = $rincianlama->RINCIAN_HARGA;
                $arcRincian->RINCIAN_LOCK           = $rincianlama->RINCIAN_LOCK;
                $arcRincian->CREATED_AT             = Carbon\Carbon::now();
                $arcRincian->TAHAPAN_ID             = $tahapan;
                $arcRincian->save();
            }
            $log        = new Log;
            $log->LOG_TIME                          = Carbon\Carbon::now();
    ...
    public function restoreRincianCBperubahan($tahun,$status){
        $id             = Input::get('RINCIAN_ID');
        foreach ($id as $id) {         
            $arcRincian       = RincianArsipPerubahan::where('ARSIP_ID',$id)->first();   
            $rincianlama       = RincianPerubahan::where('RINCIAN_ID',$arcRincian->RINCIAN_ID)->first();
            RincianPerubahan::where('RINCIAN_ID',$arcRincian->RINCIAN_ID)->delete();
            $rincian           = new RincianPerubahan;
            ...
            RincianArsipPerubahan::where('ARSIP_ID',$id)->delete();
            if($rincianlama){
                $arcRincian     = new RincianArsipPerubahan;
                $arcRincian->RINCIAN_ID             = $rincianlama->RINCIAN_ID;
                $arcRincian->SUBRINCIAN_ID          = $rincianlama->SUBRINCIAN_ID;
    ...
    public function getKomponenTerpakai($tahun, $status, $id,$blid){
        if($status=='murni'){
...
<li><a onclick="return staff(\''.$data->BL_ID.'\')"><i class="icon-bdg_people m-r-xs"></i> Atur Staff</a></li>';*/
    ...
    public function getrekeningbl($tahun,$status,$id,$rek_id){
        $data   = RincianPerubahan::join('BUDGETING.DAT_BL_PERUBAHAN','DAT_BL_PERUBAHAN.BL_ID','=','DAT_RINCIAN_PERUBAHAN.BL_ID')->where('BL_TAHUN',$tahun)->where('BL_DELETED',0)->where('REKENING_ID',$rek_id)->where('DAT_BL_PERUBAHAN.BL_ID',$id)
                        ->groupBy('REKENING_ID')
                        ->select('REKENING_ID')
                        ->selectRaw('SUM("RINCIAN_TOTAL") as total')
                        ->get();
        $data1  = RincianPerubahan::where('BL_ID',$id)->where('REKENING_ID',$rek_id)->selectRaw('DISTINCT("REKENING_ID")')->get()->toArray();
        $data_  = Rincian::join('BUDGETING.DAT_BL','DAT_BL.BL_ID','=','DAT_RINCIAN.BL_ID')->where('REKENING_ID',$rek_id)->where('BL_TAHUN',$tahun)->where('BL_DELETED',0)->where('DAT_BL.BL_ID',$id)
                        ->whereNotIn('REKENING_ID',$data1)
                        ->groupBy('REKENING_ID')
                        ->select('REKENING_ID')
                        ->selectRaw('SUM("RINCIAN_TOTAL") as total')
