...
class blController extends Controller
{
    ...
    public function showMurni($tahun,$status){
        ...
        if(Auth::user()->level == 8 or Auth::user()->level == 0){
    ...
    public function showPerubahan($tahun,$status){
        ...
        if(Auth::user()->level == 1 or Auth::user()->level == 2){
            ...
            $pagu      = SKPD::where('SKPD_ID',$id)->value('SKPD_PAGU');
            $rincian   = RincianPerubahan::whereHas('blperubahan',function($r) use($id){
                            $r->whereHas('subunit',function($s) use ($id){
                                    $s->where('SKPD_ID',$id);
                            });
                        })->sum('RINCIAN_TOTAL');
    ...
    public function showRincianPerubahan($tahun,$status,$id){
        ...
        $data       = RincianPerubahan::where('BL_ID',$id)->get();
        $data1       = RincianPerubahan::where('BL_ID',$id)->select('RINCIAN_ID')->get()->toArray();
        ...
        foreach ($data as $data) {
            ...
            $rinciansebelum     = Rincian::where('RINCIAN_ID',$data->RINCIAN_ID)->first();
            if($rinciansebelum){
    ...
    public function submitRincianPerubahan($tahun,$status){
        ...
        $nowOPD     = RincianPerubahan::whereHas('blperubahan',function($q) use($skpd){
                            $q->whereHas('subunit',function($r) use($skpd){
                                $r->where('SKPD_ID',$skpd);
                            });
                        })->sum('RINCIAN_TOTAL');
    ...
    public function getDataMurni($tahun,$status,$filter){
        ...
        foreach ($data as $data) {
            ...
            //     }else{
            //         $kunci     = '<span class="text-success"><i class="fa fa-unlock"></i></span>';
            //     }
            ...
            //     }else{
            //         $kunci      = '<span class="text-danger"><i class="fa fa-lock"></i></span>';
            //     }             
            //     if(Auth::user()->level == 4){
            ...
            if(Auth::user()->level == 2 or Auth::user()->level == 8 or substr(Auth::user()->mod,1,1) == 1){
