...
class lampiranController extends Controller
{
    ...
    public function rka($tahun, $status, $id){
        ...
        $total_murni    = Rincian::where('BL_ID',$id)->whereIn('SUBRINCIAN_ID',$subrinci)->sum('RINCIAN_TOTAL');
    ...
    public function rkpd($tahun,$status,$id){
        ...
        if($status == 'murni'){
            return View('budgeting.lampiran.rkpd',['tahun'=>$tahun,'status'=>$status,'skpd'=>$idSKPD,'pagu'=>$pagu1,'program'=>$program,'i'=>0,'paguprogram'=>$paguprogram,'urusankode'=>'xxx','bidangkode'=>'xxx']);
        }else{
