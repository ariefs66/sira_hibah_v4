...
class fungsiController extends Controller
{
    public function index($tahun,$status){
    	return View('budgeting.referensi.fungsi',['tahun'=>$tahun,'status'=>$status]);
    }

    public function getData($tahun){
    	$data 			= UrusanKategori2::where('URUSAN_KAT2_TAHUN',$tahun)->orderBy('URUSAN_KAT2_KODE')->get();
    	$no 			= 1;
    	$aksi 			= '';
    	$view 			= array();
    	foreach ($data as $data) {
    		...
    		array_push($view, array( 'no'			=>$no,
                                     'URUSAN_TAHUN'  =>$data->URUSAN_KAT2_TAHUN,
                                     'URUSAN_KODE'  =>$data->URUSAN_KAT2_KODE,
                                     'URUSAN_NAMA'	=>$data->URUSAN_KAT2_NAMA,
                                     'aksi'			=>$aksi));
    		$no++;
    	}
