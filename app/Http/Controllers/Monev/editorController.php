...
class editorController extends Controller
{
   ...
   public function getTriwulan1($tahun,$filter){
    $data        = Monev_Program::where('SKPD_ID',$filter)->where('PROGRAM_TAHUN',$tahun)
                  ->leftJoin('REFERENSI.REF_SATUAN','REF_SATUAN.SATUAN_ID','=','DAT_PROGRAM.SATUAN')
                  ->leftJoin('REFERENSI.REF_PROGRAM','REF_PROGRAM.PROGRAM_ID','=','DAT_PROGRAM.REF_PROGRAM_ID')
                    ->leftJoin('REFERENSI.REF_OUTCOME','DAT_PROGRAM.REF_PROGRAM_ID','=','REF_OUTCOME.PROGRAM_ID')
                    ->where('BL_TAHUN',$tahun)
                    ->where('BL_DELETED',0)
                    ->where('SKPD_ID',$filter)
                    ->groupBy("REF_PROGRAM".'PROGRAM_NAMA',"REF_PROGRAM.PROGRAM_ID","PROGRAM_KODE","OUTCOME_TOLAK_UKUR","OUTCOME_TARGET","SATUAN_NAMA")
                    ->selectRaw('"PROGRAM_ANGGARAN" AS TOTAL, "REF_PROGRAM"."PROGRAM_NAMA","REF_PROGRAM"."PROGRAM_ID","PROGRAM_KODE","OUTCOME_TOLAK_UKUR","OUTCOME_TARGET","SATUAN_NAMA"')
                    ->get();          
