...
class editorController extends Controller
{
    ...
    public function getTriwulan1($tahun, $filter)
    {
        ...
        foreach ($data as $data) {
            ...
            if ($monev->count()) {
                $status = '<span class="text-success"><i class="fa fa-check"> Sudah Diinput</i></span>';
            } else {
                $status = '<span class="text-danger"><i class="fa fa-close"> Belum Diinput</i></span>';
            }
    ...
    public function getTriwulan3($tahun, $filter)
    {
        ...
        foreach ($data as $data) {
            ...
            array_push($view, array('ID' => $opsi,
                ...
                'OUTCOME' => $data->OUTCOME_TOLAK_UKUR,
                'TARGET' => $data->OUTCOME_TARGET . ' ' . $data->SATUAN_NAMA,
                'KINERJA' => $monev->value('PROGRAM_T3') . ' ' . $monev->value('SATUAN_NAMA'),
                'STATUS' => $status,
                'TOTAL' => number_format($data->total, 0, '.', ',')));
            $no++;
        }
    ...
    public function getTriwulan4($tahun, $filter)
    {
        ...
        foreach ($data as $data) {
            ...
            array_push($view, array('ID' => $opsi,
                ...
                'OUTCOME' => $data->OUTCOME_TOLAK_UKUR,
                'TARGET' => $data->OUTCOME_TARGET . ' ' . $data->SATUAN_NAMA,
                'KINERJA' => $monev->value('PROGRAM_T4') . ' ' . $monev->value('SATUAN_NAMA'),
                'STATUS' => $status,
                'TOTAL' => number_format($data->total, 0, '.', ',')));
            $no++;
        }
    ...
    public function getData($tahun, $skpd, $mode = 1, $id)
    {
        ...
        $data = Monev_Kegiatan::where('KEGIATAN_ID', $data->KEGIATAN_ID)->get();
    ...
    public function getDetail($tahun, $skpd, $mode = 1, $id=0)
    {
        ...
        $akb = '';
        foreach ($data as $data) {
            $output = Monev_Output::where('KEGIATAN_ID', $data->KEGIATAN_ID)->count();
            ...
            $output_bl = $output;
            if ($sub_id) {
                $bl = BLPerubahan::where('KEGIATAN_ID', $data->REF_KEGIATAN_ID)->where('SKPD_ID', $data->REF_KEGIATAN_ID)->where('SUB_ID', $sub_id)->first();
                $output_bl = $OutputPerubahan::where('SKPD_ID', $skpd)->where('BL_ID', $bl->BL_ID)->where('SUB_ID', $sub_id)->count();
            }
            if ($output_bl == $output) {
                $status = '<span class="text-success"><i class="fa fa-check"> ' . $output . ' Output</i></span>';
            } else {
                $status = '<span class="text-success"><i class="fa fa-close"> ' . $output . ' Output</i></span>';
            }
            $opsi = '<div class="dropdown dropdown-blend" style="float:right;"><a class="dropdown-toggle" type="button" id="dropdownMenu2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="text text-success"><i class="fa fa-chevron-down"></i></span></a><ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
            ...
            array_push($view, array('NO' => $no++,
                ...
                'KINERJA' => ' ',
                'TOTAL' => number_format($data->EKGIATAN_ANGGARAN, 0, '.', ','),
                'AKSI' => $opsi,
                'STATUS' => $status));
        }
